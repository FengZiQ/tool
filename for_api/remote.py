...
class Server(object):
    ...
    def webapi(self, method, serverIp, service, parameters=None):

        url = serverIp + "/" + service
        header = dict()
    ...
    def webapiurl(self, method, serverIp, service, urlparameter):

        url = "https://" + serverIp + "/" + service + '/' + urlparameter
        header = dict()
        header["Content-Type"] = "application/json"
        header["additioninfo"] = self.addition_info
