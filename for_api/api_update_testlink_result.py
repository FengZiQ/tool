        ...
        for case_body in case.values():
            ...
            if not case_body['exec_on_build']:
                ...
                if list(tester_key.keys())[0] == login_name[0]['login']:
                    ...
                    temp3 = temp1[-2].replace("'p'", '').replace("'f'", '')
                    temp4 = temp3.split('-*- The case is executed! -*-')[:-1]

                    steps_notes = []
                    for temp5 in temp4:
                        a = re.search('HTTP Status.*]>', temp5, re.I)
                        b = re.search('Actual Response:.*', temp5, re.I)
                        if a and '500' not in a.group():
                            steps_notes.append(a.group() + '\n' + b.group().replace('Actual Response:', ''))
                        else:
                            steps_notes.append(a.group())
